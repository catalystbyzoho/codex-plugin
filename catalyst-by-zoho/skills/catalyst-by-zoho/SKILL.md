...
- **Request approval for Catalyst CLI execution that escapes the workspace.** For Catalyst CLI commands that need network access or write home-directory/global CLI state — including `catalyst login`, `catalyst init`, `catalyst deploy`, dependency install/update flows, and `catalyst config:set` — request escalated execution. Explain that the command may contact Catalyst/Zoho services and write credentials, project links, or CLI config outside the workspace.
...
**Codex approval:** Before running Catalyst CLI commands that need network access or home-directory/global CLI writes, request escalated execution. This applies to `catalyst login`, `catalyst init`, `catalyst deploy`, dependency install/update flows, and `catalyst config:set`.
...
- **Escalate Catalyst CLI commands that use network or global config** — request approval for `catalyst login`, `catalyst init`, `catalyst deploy`, dependency install/update flows, and `catalyst config:set`
