 ...
 * session start, so Codex begins development already knowing the workspace it is
...
function findCatalystMcp(projectRoot) {
  ...
  add(process.cwd(), 'cwd');
  add(process.env.PLUGIN_ROOT || process.env.CLAUDE_PLUGIN_ROOT, 'plugin');
...
// ---------------------------------------------------------------------------
// Prerequisite checks — returns { issues: [{level, user, codex}] }
...
function runPrereqChecks(root) {
  ...
  if (nodeRaw === null) {
    issues.push({
      ...
      codex: 'Node.js is not on PATH. The Catalyst CLI cannot run. Guide the user to install Node 20+ before any CLI/scaffold step.',
    });
    ...
    issues.push({
      ...
      codex: `Node.js ${nodeRaw} is below the CLI minimum (Node ${MIN_NODE_MAJOR}+). CLI commands may fail; guide the user to upgrade Node before scaffolding/deploying.`,
    });
  }
  ...
  if (cliRaw === null) {
    ...
    if (cliPath) {
      ...
      issues.push({
        ...
        codex: `The Catalyst CLI binary exists (${where}) but 'catalyst -v' failed — likely a broken install (or a transient error). Do not assume CLI commands will work; if commands keep failing, guide the user to reinstall/repair before scaffolding or deploying.`,
      });
    } else {
      issues.push({
        level: 'blocker',
        user: "Catalyst CLI not found on PATH — install with: npm i -g zcatalyst-cli (needs Node 20+).",
        codex: 'The Catalyst CLI is not installed / not on PATH. Do NOT attempt catalyst commands; guide the user to run `npm i -g zcatalyst-cli` first.',
      });
    }
  } else if (!cliVer) {
    issues.push({
      ...
      codex: `Catalyst CLI version could not be parsed ("${cliRaw}"). Verify it is >= ${MIN_CLI.join('.')} before relying on -ni commands.`,
    });
    ...
    issues.push({
      ...
      codex: `Catalyst CLI is v${cliVer.join('.')}, below v${MIN_CLI.join('.')}. Non-interactive (-ni) commands (init, functions:add, etc.) are unavailable — either guide the user to upgrade, or fall back to the documented interactive flow. Do not emit -ni commands as if they will work.`,
    });
  }
  ...
  if (cliRaw !== null) {
    ...
    if (loggedOut) {
      issues.push({
        ...
        codex: 'The Catalyst CLI is not authenticated (`catalyst whoami` reports no active session). CLI cloud operations (init, deploy, functions:add, pull) will fail. Guide the user to run `catalyst login` before attempting them; do not emit those commands expecting success.',
      });
    }
  ...
  if (need.node > 0) {
    ...
    if (configuredButMissing(cfg)) {
      issues.push({
        ...
        codex: `node${need.node} is mapped via CLI config '${key}.bin' to a missing path (${cfg}); local serve/execute for that component will fail until the path is corrected or node${need.node} is on PATH.`,
      });
      ...
      issues.push({
        ...
        codex: `A component declares node${need.node}; installed Node is ${nodeRaw} with no CLI '${key}.bin' override. Local execution may diverge from / fail against the deploy runtime.`,
      });
    }
  }
  ...
  if (need.java > 0) {
    ...
    if (configuredButMissing(cfg)) {
      issues.push({
        ...
        codex: `java${need.java} is mapped via CLI config '${key}.bin' to a missing path (${cfg}); serve/execute for that component will fail until corrected.`,
      });
      ...
      issues.push({
        ...
        codex: `A Java stack (java${need.java}) is declared locally, 'java' is not on PATH, and no CLI '${key}.bin' override exists. catalyst serve / functions:execute for that component will fail.`,
      });
    }
  }
  ...
  if (need.python > 0) {
    ...
    if (configuredButMissing(cfg)) {
      issues.push({
        ...
        codex: `${pyLabel} is mapped via CLI config '${key}.bin' to a missing path (${cfg}); serve/execute for that component will fail until corrected.`,
      });
      ...
      issues.push({
        ...
        codex: `A Python stack (${pyLabel}) is declared locally, no python3 is on PATH, and no CLI '${key}.bin' override exists. catalyst serve / functions:execute for that component will fail.`,
      });
    }
  }
  ...
  if (hasDockerfile) {
    ...
    if (dockerRaw === null || /error|cannot connect|refused/i.test(dockerRaw)) {
      issues.push({
        ...
        codex: 'A Dockerfile (custom runtime) exists but the Docker daemon is not reachable. Building or deploying the custom-runtime AppSail will fail until Docker is running and accessible.',
      });
    }
  }
...
function main() {
  ...
  if (issues.length > 0) {
    ...
    for (const i of blockers) lines.push(`- [BLOCKER] ${i.codex}`);
    for (const i of warns) lines.push(`- [WARN] ${i.codex}`);
  }
